  ...
  case instructions !? instructionPointer of
    ...
    Just i ->
      ...
      in case i of
           ...
           Send o -> step $ state {outgoing = push outgoing $ valOf o}
           Receive r ->
...
solve' instructions s@State {..} =
  case eval instructions running of
    (s', JumpOutside) -> undefined -- s {running = s'}
    (s', EmptyReceive) -> undefined -- solve' instructions $ s {}
    _ -> undefined

solve :: [Instruction] -> State
solve ins = solve' (V.fromList ins) initial

readAndSolve :: String -> Either String State
