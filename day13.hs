...
solve :: [Firewall] -> Int
solve fws' = helper 0 fws' 0
  where
    helper :: Int -> [Firewall] -> Int -> Int
    helper _ [] res = res
    helper depth (fw:fws) res = helper (depth + 1) (map tick nextFws) nextRes
      where
        ...
        nextRes =
          if fwIsForDepth && fwPosition fw == 0
            then res + fwDepth fw * fwRange fw
            else res
...
tests = do
  ...
  describe "solve" $
    ...
    solve <$> readFirewalls example `shouldBe` Right 24
