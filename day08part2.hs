...
type State = Map.Map Register (Int, Int)

getRegister :: State -> Register -> Int
getRegister s r = fst $ Map.findWithDefault (0, 0) r s

evalOperation :: Register -> Operation -> State -> State
evalOperation r op = Map.alter eval r
  where
    eval mv = Just (nv, max m nv)
      where
        (v, m) = fromMaybe (0, 0) mv
        nv =
          case op of
            Increase x -> v + x
            Decrease x -> v - x
...
findMax s =
  switch $
  Map.findMax $ Map.fromList $ (\(n, (_, m)) -> (m, n)) <$> Map.toList s
