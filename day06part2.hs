...
-- not total, i.e. totally cheating
diffIndex :: Eq a => a -> [a] -> Int
diffIndex x xs = i2 - i1
  where
    [i1, i2] = elemIndices x xs

solve :: [Int] -> Int
solve initial = diffIndex cycleElem loop
  where
    loop = helper initial []
    cycleElem = last loop
...
tests = do
  describe "diffIndex" $
    it "works" $ diffIndex (1 :: Int) [0, 1, 0, 0, 0, 1] `shouldBe` 4
  ...
  describe "solve" $ it "works with example" $ solve [0, 2, 7, 0] `shouldBe` 4
