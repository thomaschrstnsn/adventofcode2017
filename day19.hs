...
collect :: Input -> State -> String -> String
collect inp state res =
  if shouldCollect x
    then res ++ [x]
    else res
  where
...
follow :: Input -> State -> String -> String
follow inp state res =
  case continueState of
    ...
    Nothing -> res'
  where
    res' = collect inp state res
    continueState = continue inp state

solve :: Input -> String
solve inp = follow inp (findStart inp) ""
...
tests = do
  describe "solve" $
    ...
    (solve . readInput) example `shouldBe` "ABCDEF"
  describe "findStart" $
