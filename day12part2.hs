...
findDisconnectedGroups ::
  ...
  -> Set (Set Program)
  -> Set (Set Program)
findDisconnectedGroups connections allPrograms groups =
  if Set.null notInGroupYet
    then groups
    else findDisconnectedGroups
           connections
           allPrograms
           (Set.union groups $
            Set.singleton $
            convergingConnectedTo connections $
            Set.singleton $ Set.findMin notInGroupYet)
  where
    notInGroupYet = Set.difference allPrograms $ Set.unions (Set.toList groups)

findAllPrograms :: [ProgramConnection] -> Set Program
findAllPrograms pcs = Set.fromList $ fst <$> pcs

solve :: [ProgramConnection] -> Int
solve pcs =
  Set.size $ findDisconnectedGroups pcs (findAllPrograms pcs) Set.empty
...
tests =
  ...
  solve <$> readProgramConnections example `shouldBe` Right 2
