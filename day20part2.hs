...
collisions ps = filter (\p -> position p `notElem` colliders) ps
  where
    colliders :: [Vector]
    colliders = map fst $ filter (\(_, i) -> i >= 2) $ freq $ position <$> ps

updateMany :: [Particle] -> [Particle]
updateMany = collisions . map update

solve :: [Particle] -> Int
solve ps = length $ head $ drop 1000 $ iterate updateMany ps
